      ...
      {value.icon ? (
        value.name === "HTMLs" ? (
          <FontAwesomeIcon
            ...
            style={{
              color: value.color,
              fontSize: screenSize >= 865 ? "95px" : "45px",
            }}
          />
        ) : (
          <value.icon
        ...
        <>
          <Vscode size={screenSize >= 865 ? 85 : 35} />
          <div className="h-"></div>
        </>
      )}

      <h3 className="  text-[0.5rem]  sm:text-xs text-wrap  md:text-sm lg:text-base">
        {value.name}
      </h3>
      <h2
        className={` text-[0.5rem]   sm:text-xs text-wrap  md:text-sm lg:text-base text-[#152256]  translate-y-0.5 sm:translate-y-1  invert font-semibold`}
      >
        {progress + "%"}
      </h2>
    </div>
